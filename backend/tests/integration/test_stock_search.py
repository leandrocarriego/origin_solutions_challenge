"""GET /api/stocks: the autocomplete of the catalogue.
...
- **What matches.** Symbol, name, and neither of the two cares about case in
  either direction: the catalogue stores `MSFT` and `Microsoft Corp`, and a person types
  whatever they type.
- **Twenty at most, chosen by relevance**. The cap only proves something with more than
...
leaves the suggestions, while staying in the grid of whoever already had it.
"""
...
# More than the twenty of the cap, so it has something to cut. Their names carry `micro`
...
async def catalogue(session: AsyncSession) -> None:
    ...
    The rest are the edges: a delisted symbol, a name with an underscore in it, and one
...
class TestWhatTheTextMatches:
    """Symbol, name, and case in neither direction."""

    async def test_a_piece_of_the_symbol_finds_the_stock(
        self, client: AsyncClient, juan: User, catalogue: None
    ) -> None:
        """Typing part of the symbol is the fastest way to reach a known company."""
    ...
    async def test_a_piece_of_the_name_finds_the_stock(
        ...
        """Somebody who knows the company and not its ticker still finds it."""
    ...
    async def test_lower_case_finds_an_upper_case_symbol(
        ...
        """One direction: the catalogue stores `MSFT` and nobody types in caps."""
    ...
    async def test_upper_case_finds_a_mixed_case_name(
        ...
        """The other direction, and the one an `.upper()` on the text would break.
    ...
    async def test_an_anonymous_call_is_refused(self, client: AsyncClient) -> None:
        """The catalogue is not user data, and it is not public either."""
...
class TestHowManyComeBackAndWhich:
    """the relevance: twenty at most, and the right twenty."""

    async def test_a_very_common_text_answers_twenty(
        self, client: AsyncClient, juan: User, catalogue: None
    ) -> None:
        """Twenty-five rows match `micro`; a dropdown holds twenty."""
    ...
    async def test_micro_brings_microsoft_among_the_twenty(
        ...
        """The acceptance criterion, and the test that tells the two layers apart.
...
class TestASymbolThatStoppedTrading:
    """It leaves the suggestions, and only the suggestions."""
...
class TestHowShortATextIsRejected:
    """the asymmetry between the router and the service.
    ...
    async def test_one_character_is_a_422(
        ...
        """The router refuses it before any service exists."""
    ...
    async def test_two_spaces_are_an_empty_list_and_not_a_422(
        ...
        cost the minimum exists to avoid, paid by a query the minimum never saw.
        """
