"""The four states, over HTTP: all of them 200, and none of them naming anybody (ERR-05, RF-26).
...
a 429 forwarded to the browser blames a user who has no account with any provider (ERR-05). The
same for the name: the service can be perfectly generic and a handler can still leak the
provider into an error body (Article I, RF-26).
...
# The four ways the world can refuse to answer (ADR-006). Not one of them is an error of ours.
_FAILURES = [
...
async def _follow(session: AsyncSession, user_id: int, symbol: str) -> None:
    """Make the symbol one of that user's favourites, catalogue row included (RF-35)."""
...
class TestADayWithNoSession:
    """RF-27 and RF-28: the Saturday of the demo, charted with the last session there was."""
    ...
    async def test_it_charts_that_earlier_session(
        ...
        """RF-27: the last session available, and not a blank screen."""
    ...
    async def test_it_names_the_day_that_session_traded_in_market_time(
        ...
        """RF-36: a session that crosses midnight in UTC is still one day where it happened."""
        opened = await _an_earlier_session(session)
...
class TestNothingAnywhere:
    """RF-31: no series for that symbol and interval, which is not the same as a closed market."""
    ...
    async def test_no_data_carries_no_points(
        ...
        """RF-33: the screen has to tell an empty chart from a chart it never drew."""
        response = await _chart(client, juan)
...
class TestAProviderThatFailed:
    """RF-29, RF-30 and ERR-05: the failure is answered, and it is answered as `stale`."""
...
class TestEveryStateAnswers200:
    """ERR-05: a failure upstream never reaches the browser as a failure of ours."""
