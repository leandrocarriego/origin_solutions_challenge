"""POST /api/auth/login, against the real table.
...
. Comparing bodies is not enough: a `WWW-Authenticate` on one and not the other tells
  the caller which username exists.
- **Nothing on the way out carries the clave**: not the body, not the token's payload,
...
# is one per request by definition, and it says nothing about the credential.
...
class TestACredentialThatIsGood:
    """What the screen needs to store a session and paint the header."""
    ...
    async def test_the_token_is_issued_for_the_user_that_logged_in(
        ...
        """The `sub` is the identity every protected route will read."""
        response = await client.post(_LOGIN, json={"username": "juan", "password": PASSWORD})
    ...
    async def test_the_username_is_matched_whatever_the_case(
        ...
        """In SQL: the column is not case-insensitive, so the query has to be."""
...
class TestACredentialThatIsNot:
    ...
    async def test_the_password_is_case_sensitive(self, client: AsyncClient, juan: User) -> None:
        """The username folds case, the clave does not."""
    ...
    async def test_an_unknown_user_answers_exactly_like_a_wrong_password(
        ...
        """Status, body and headers identical, or the difference enumerates usernames."""
...
class TestNothingOnTheWayOutCarriesTheClave:
    """at the only endpoint of the project that is handed a password."""
    ...
    async def test_the_logs_of_a_rejected_attempt_do_not_carry_it(
        ...
        """A clave in a log defeats the hashing of the column that stores it."""
...
class TestABodyThatIsNotOne:
    """422 is the backstop: the screen validates the empty fields before asking."""
