...
class _Store:
    """A `UserStore` that answers a fixed row and remembers what it was asked for."""
...
def repository() -> _Store:
    """A store that finds the demo user."""
    return _Store(user=_juan())


@pytest.fixture
def no_such_user() -> _Store:
    """A store that finds nothing, for the half of the refusal that has no row."""
    return _Store(user=None)
...
async def _attempt(
    ...
    return await authenticate(store, username=username, password=password, client_ip=client_ip)
...
class TestWhatTheUserNameMeans:
    ...
    async def test_the_password_is_case_sensitive(self, repository: _Store) -> None:
        """Relaxing the username is a convenience; relaxing the password removes entropy."""
        with pytest.raises(AuthenticationError):
            await _attempt(repository, password=PASSWORD.upper())
    ...
    async def test_a_password_that_is_merely_close_does_not_get_in(
        ...
        with pytest.raises(AuthenticationError):
            await _attempt(repository, password=PASSWORD[:-1])
...
class TestTheTwoFailuresAreOneFailure:
    ...
    async def test_an_unknown_user_raises_the_authentication_error(
        ...
        with pytest.raises(AuthenticationError):
            await _attempt(no_such_user, username="nadie")

    async def test_both_failures_raise_the_very_same_thing(self) -> None:
        """Same type and same message: a subclass or an extra word is the leak."""
        with pytest.raises(AuthenticationError) as no_row:
            await _attempt(_Store(user=None), username="nadie")

        with pytest.raises(AuthenticationError) as bad_password:
            await _attempt(_Store(user=_juan()), password="otra-clave")
    ...
    async def test_an_unknown_user_is_still_checked_against_a_hash(
        self, no_such_user: _Store, password_check: _PasswordCheck
    ) -> None:
...
class TestNothingWrittenDownCarriesThePassword:
    ...
    async def test_a_failed_attempt_logs_the_attempt_and_not_the_password(
        ...
        with pytest.raises(AuthenticationError):
            await _attempt(repository, password="otra-clave-mal-tipeada")
...
class TestTheAttemptLimit:
    ...
    async def test_the_attempt_after_the_limit_is_refused(self, repository: _Store) -> None:
    ...
    async def test_even_the_right_password_is_refused(self, repository: _Store) -> None:
        ...
        with pytest.raises(RateLimitedError):
            await _attempt(repository, password=PASSWORD)

    async def test_the_right_password_is_not_even_verified(
        self, repository: _Store, password_check: _PasswordCheck
    ) -> None:
    ...
    async def test_the_refusal_says_how_long_to_wait(self, repository: _Store) -> None:
    ...
    async def test_a_username_that_does_not_exist_is_counted_too(
        ...
        for attempt in range(LOGIN_MAX_ATTEMPTS):
            with pytest.raises(AuthenticationError):
                await _attempt(no_such_user, username="nadie", client_ip=f"198.51.100.{attempt}")

        with pytest.raises(RateLimitedError):
            await _attempt(no_such_user, username="nadie", client_ip="198.51.100.200")
