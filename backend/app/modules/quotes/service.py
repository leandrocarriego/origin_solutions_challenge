...
The quota is 800 requests a day and a session at `1min` has 390 candles, so the rule is not
"cache when convenient": nothing reaches the provider if the database can answer, and what does
reach it is shared. Two things make that true, and they are different:
...
- **the gate** stops a second reader that arrives while the first one is still fetching, which a
  ...
  browsers land in the same second.

The provider arrives as an argument and is never built here: that is what lets the whole suite
run with no network and no API key, and it is also why this file can be read without knowing
who the provider is.
"""
...
# what the screen needs on a Sunday with an empty database.
REALTIME_LOOKBACK = timedelta(days=7)

# The TTL of a candle is the interval it covers: before the next one closes there is
...
# How long a window may be, per interval. They live here and nowhere else: a
...
class _Gatekeeper:
    ...
    The pair is the key, and that is what makes the quota scale with symbols observed: ten
    browsers on TSLA at `5min` are one question and cost one credit, and the same ten on `1min`
    are another one.
...
async def get_series(
    ...
    `user_id` is the identity of the token and the only one this ever sees, and
    ...
    **The range is validated first**, before authorizing and before a single query: a query that
    cannot be asked is not executed, and "not executed" includes the lookup of whose symbol this
    is. The consequence is deliberate -- somebody else's symbol asked for with an
...
def _audit(
    ...
    One line per request and not only per call to the provider: what has to be auditable includes
    **whether the provider was reached at all**, and a request served from the database left no
    trace before this -- so the log answered "how often do we spend a credit?" with silence on
    the numerator's other half. The counters carry the aggregate; this carries the request.

    Who the provider is stays out of it, here as everywhere.
    """
    _log.info(
        "quote_served",
        symbol=symbol,
...
def _window_asked_for(
    ...
    they become instants, because the market is a fact of the product and not of the request. A
    service that stamped UTC on them would answer a window four or five hours away from the one
    that was pointed at, with every point still neatly on the axis.
    """
...
def _today_began(now: datetime) -> datetime:
    ...
    and a window that started there would ask for a session that has not opened.
    """
...
def _answers_already(
    ...
    while the candle in progress has not closed, so the TTL is the interval: asking
...
async def _fill(
    ...
    **Which window goes out is decided here, and there are two of them in `Tiempo Real`.** With
    ...
    last session there was, which is what the screen needs on a Sunday. In `Histórico`
...
async def _fetch(
    ...
    and every one of them is logged with the window it happened in. What is never logged is who
    the provider is.
...
async def _answer(
    ...
    inventing the first. So a failed fetch is `stale`, always.

    When today came out empty the chart falls back to the last session there is: on a Saturday
    the screen shows Friday with a notice, and not a blank rectangle. That fallback belongs to
    `Tiempo Real` alone -- somebody who asked for a window of June wants that window, and
    answering another day's prices would be a different question answered quietly.
    """
    if stored:
        return QuoteSeries(
...
async def _last_session(
    ...
    runs to 21:30 in New York is already the next day in UTC, and the notice would name a day
    that never traded.
    """
