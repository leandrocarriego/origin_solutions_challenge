...
- composición HTTP (el `lifespan`, los middlewares y el montaje de cada router) → `app/main.py`, y
  lo que corre de fondo → `app/tasks.py`. Son los **dos únicos** archivos por debajo de los módulos
  que pueden importar un módulo (`GEN-03`), y esa lista la fija un test: un tercero se agrega a
  propósito o no se agrega.
