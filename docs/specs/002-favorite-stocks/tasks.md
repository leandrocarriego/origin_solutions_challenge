...
**Tests aprobados por:** Leandro Carriego *(H1, H2, H3, H4, y los cuatro de regresión del review)* · **Fecha de aprobación:** 2026-09-14 *(todas)*

| Historia | Tests | Firma | Fecha |
|---|---|---|---|
| H1 — Ver mis acciones | tareas 1 y 2 | Leandro Carriego | 2026-09-14 |
| H2 — Agregar una acción a mi lista | tareas 8 y 9 | Leandro Carriego | 2026-09-14 |
| H3 — Sacar una acción de mi lista | tareas 14 y 15 | Leandro Carriego | 2026-09-14 |
| H4 — Llegar al detalle de una acción | tarea 19 | Leandro Carriego | 2026-09-14 |
...
> **Re-firma del 2026-09-14 — dos tests del alta de H2.** Durante `/implement` apareció que un
> test firmado y el `plan.md` se contradecían: el test mandaba `{"symbol": "  msft "}` esperando
> **201**, y el patrón que *Contratos* fija para el body rechaza los espacios con **422** antes de
> que exista ningún service. Ninguna implementación podía cumplir las dos cosas. El humano decidió
> que **gana el plan**, el `Tester` corrigió, y Leandro Carriego volvió a firmar los dos tests
> resultantes en `backend/tests/integration/test_favorites_add.py`:
> `test_the_symbol_is_normalised_before_anything_else` —`{"symbol": "msft"}` → **201** con `MSFT`,
> que es la mitad cierta del original— y su hermano nuevo
> `test_a_symbol_with_spaces_around_it_never_reaches_the_service` —`{"symbol": " msft "}` → **422**
> sin escribir ninguna fila—.
>
> Queda anotado porque es exactamente lo que el Artículo VI pide: un test firmado que resulta
> equivocado **se corrige y se vuelve a firmar**, nunca se ajusta al código recién escrito. La
> frase del plan que quedó vieja se corrigió aparte (`plan.md` → precondición de `get_stocks`).

> **Firma del 2026-09-14 — cuatro tests de regresión del review.** El `/review-feature` encontró
> dos defectos y el humano pidió corregirlos, así que los tests llegaron **después** de la
> implementación que los pone en verde: es la excepción que el Artículo VI contempla para una
> corrección, no el orden de una historia. Leandro Carriego los firmó el 2026-09-14, y son cuatro:
> en `frontend/tests/addFavorite.test.tsx`, los tres de `the dropdown once a suggestion was chosen`
> —queda cerrado, no vuelve a preguntarle a la API, y reaparece apenas alguien tipea de nuevo— y
> en `frontend/tests/MyActions.test.tsx`, `puts that notice inside the grid, where the rows would
> be`, que fija lo que `COPY.md` ya decía: el aviso de la lista vacía va **en lugar de las filas**.
>
> Los dos primeros fallaban antes del arreglo, que es lo que los hace tests y no decoración.
...
| RF-15 | 8, 9, 12, 13 | `backend/tests/integration/test_favorites_add.py` · `frontend/tests/addFavorite.test.tsx` |
| RF-16 | 9, 13 | `frontend/tests/MyActions.test.tsx` |
| RF-17 | 8, 9, 12, 13 | `backend/tests/integration/test_favorites_add.py` · `frontend/tests/addFavorite.test.tsx` |
| RF-18 | 8, 12 | `backend/tests/integration/test_favorites_add.py` |
...
| RF-23 | 15, 18 | `frontend/tests/removeFavorite.test.tsx` · `frontend/tests/copy.test.ts` |
| RF-24 | 14, 15, 17 | `backend/tests/integration/test_favorites_remove.py` · `frontend/tests/removeFavorite.test.tsx` |
| RF-25 | 15, 18 | `frontend/tests/removeFavorite.test.tsx` · `frontend/tests/copy.test.ts` |
| RF-26 | 15, 18 | `frontend/tests/removeFavorite.test.tsx` |
| RF-27 | 14, 17 | `backend/tests/integration/test_favorites_remove.py` |
| RF-28 | 14, 17 | `backend/tests/integration/test_user_isolation.py` |
| RF-29 | 19, 21 | `frontend/tests/stockDetail.test.tsx` |
| RF-30 | 19, 21 | `frontend/tests/stockDetail.test.tsx` |
| RF-31 | 9, 13 | `frontend/tests/MyActions.test.tsx` |
| RF-32 | 15, 18 | `frontend/tests/removeFavorite.test.tsx` |
