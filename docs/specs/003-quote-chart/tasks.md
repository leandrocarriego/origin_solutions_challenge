...
| H1 — *enmienda*: la hora del eje (`marketClockValue`) | los 5 casos nuevos de `market.test.ts` | Leandro Carriego | 2026-09-14 |
| H1–H4 — *enmienda del review*: la API que no contesta | los 21 casos de `apiUnreachable.test.tsx` y `quoteRefreshFailure.test.tsx` | Leandro Carriego | 2026-09-14 |
...
>
> **La quinta firma es una enmienda, y es la que el Artículo VI pide cuando un test cambia.**
> `/converge` levantó el hallazgo H-01 —el eje rotulaba `11/09/2026 15:55` en cada tick donde el
> wireframe 03 escribe `13:10`—, el plan agregó `marketClockValue()` a la firma de `quotes/market.ts`
> y el `Tester` escribió sus **5 casos**: la hora sola, el cambio de estación, que la lectura sea la
> de un reloj en la zona del mercado y no la de la máquina, que sea la hora con la que termina
> `formatMarket()` —así las dos no se separan— y que no lleve fecha en ninguna forma. Los cinco se
> leyeron uno por uno antes de firmar y los cinco fallaban con
> `market.marketClockValue is not a function`: ausencia de implementación, no un import roto.
>
> **La sexta firma es una enmienda del `/review-feature`, y se firmó en verde. Queda dicho.**
> Al cerrar el review el cliente decidió dos comportamientos que la spec no pide y que hasta
> entonces eran silencio: que el aviso de `stale` se muestre también cuando **falla el refresco
> automático** de `Tiempo Real`, con precedencia sobre el estado con el que llegó la serie, y que
> el Detalle reuse el `No pudimos conectarnos con el servidor. Intentá de nuevo en unos minutos.`
> del ingreso cuando no llega la serie de `Graficar` o no llega la lista de favoritas. Las dos
> decisiones están escritas en `docs/design/COPY.md` con su alternativa descartada.
>
> El `Tester` escribió sus **21 casos** en dos archivos nuevos —`quoteRefreshFailure.test.tsx` (7)
> y `apiUnreachable.test.tsx` (14)—, se leyeron uno por uno antes de firmar, y **los 21 se firmaron
> pasando en verde**: la implementación en `ActionDetail.tsx` y `Notice.tsx` ya estaba escrita
> cuando se los presentó. **El gate del Artículo VI no se cumplió para este pedazo**, y esta
> anotación existe para que eso no quede tapado: lo que se firmó acá no es "esto todavía no
> existe" sino "esto es el contrato de lo que ya existe". Se eligió registrarlo así, con la
> salvedad a la vista, sobre revertir la implementación para volver a verlos en rojo *(decidido
> por el humano, 2026-09-14)*.
>
> **Dos cosas quedan abiertas, y no las cierra esta firma.** Ninguno de los 21 responde a un `RF`
> propio: el hecho nuevo vive sólo en `COPY.md` y `spec.md` no lo pide —se apoyan en `RF-23`,
> `RF-26`, `RF-32`, `RF-35`, `RF-46`, `RF-48`, `UI-05`, `ERR-06` y `TS-06`, que ya estaban—. Y ni
> la tabla de cobertura de más abajo ni `converge.md` los mencionan todavía.
