...
> están en `CONVENTIONS.md`, que es su fuente única. Las fronteras entre módulos, las reglas del
> dominio y la anatomía de un módulo, en `AGENTS.md` y `ARCHITECTURE.md`. Acá va el mandato del
> rol; el recorrido paso a paso del review está en `agents/skills/review_feature.md`.
...
**No se copia acá.** La tabla vive sólo en `CONVENTIONS.md` → *Índice de Blockers*: son 18
hallazgos sobre 34 convenciones, y una copia en este archivo queda vieja sin que nadie lo note.
Abrilo al empezar el review y recorrelo por identificador.

Nueve de esas convenciones ya las verifica un test que rompe el build (`GEN-02`, `PY-06`, `GEN-08`,
`GEN-09`, `PY-08`, `TEST-03`, `TEST-05`, `UI-02`, `UI-03`): si la suite pasó, no hace falta revisarlas a
ojo. **El resto depende de que las recorras**, y ésa es la parte del review que no se puede
automatizar todavía. Si encontrás una
...
- **Blocker**: cualquier convención de `CONVENTIONS.md` marcada como Blocker, citada por su
  identificador.
- **Major**: mantenibilidad, manejo de errores, altas no idempotentes o falta de tests.
