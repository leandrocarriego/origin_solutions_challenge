...
- **No se inventa una skill que no existe.** Si ninguna aplica, se frena y se dice: puede faltar
  una skill, o el trabajo puede no corresponder al proyecto. En los dos casos se pregunta antes de
...
3. Anotar en cada tarea: qué hace, **la skill** que la ejecuta, el rol dueño y qué requisitos
   cubre. El índice está en `agents/skills/README.md`.
4. Ordenar dentro de cada historia: migración → **tests** → backend → frontend. Las migraciones
   primero porque todo lo demás depende del esquema; los tests antes que el código porque el
   humano los firma antes de que se implemente nada (Artículo VI). Los tests son **una tarea
   propia**, del `Tester`, no un apéndice de la tarea que los hace pasar.
...
- Tareas del tamaño de una feature ("implementar las cotizaciones").
